...
const MoreEvents: React.FC<MoreEventProps> = ({ event }) => {
  ...
  const startTimeString =
    event && event.startTime ? `${currentDate}T${event.startTime}` : 0; // Combine the date with the time
  const endTimeString =
    event && event.endTime ? `${currentDate}T${event.endTime}` : 0; // Combine the date with the time
    ...
    <Box className="more-events" sx={{ display: "flex" }}>
      ...
      >
        <Box sx={{ flex: 1 }}>
          ...
          <Typography variant="body2" color="#19d3a9" component="p">
          ...
          <Box className="creator-info">
            <Person sx={{ fontSize: "15px", color: "#19d3a9" }} />
